...
import { SendToSlicerInputSchema, AnalyzePrintabilityInputSchema, RepairPrintabilityInputSchema, ProcessMulticolorInputSchema, validateExactlyOneSource } from "../schemas/printing.js";
...
Examples:
  ...
  - From a URL:  { model_url: "https://example.com/model.glb" }`,
        ...
        try {
            ...
            const response = await client.post("/openapi/v1/print/analyze", request);
            ...
            }, params.response_format, "Printability Analysis Task Created", `Analyzing ${sourceDesc} for FDM printability. The result will report watertightness, volume, non-manifold edges, degenerate faces, and holes.`, "print-analyze");
        }
        catch (error) {
            return {
                isError: true,
    ...
    // ── Repair Printability ─────────────────────────────────────────
    server.registerTool("meshy_repair_printability", {
...
Examples:
  ...
  - From an STL: { model_url: "https://example.com/model.stl" }`,
        ...
        try {
            ...
            if (params.model_url)
                request.model_url = params.model_url;
            const response = await client.post("/openapi/v1/print/repair", request);
            const taskId = response.result;
            const sourceDesc = params.input_task_id
                ? `task "${params.input_task_id}"`
                : `model URL`;
            ...
            }, params.response_format, "Printability Repair Task Created", `Repairing ${sourceDesc}'s topology (non-manifold edges, holes, degenerate faces). Output preserves the input format; textures are not preserved.`, "print-repair");
        }
        catch (error) {
            return {
                isError: true,
...
Examples:
  ...
  - From URL: { model_url: "https://example.com/textured.glb", max_colors: 6 }`,
        ...
        try {
            ...
            }, params.response_format, "Multi-Color Processing Task Created", `Processing ${sourceDesc} into ${params.max_colors} colors (depth: ${params.max_depth}). The output will be a 3MF file for multi-color 3D printing.`, "multi-color-print");
        }
        catch (error) {
            return {
                isError: true,
